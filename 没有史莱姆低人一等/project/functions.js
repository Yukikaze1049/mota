var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = 
{
	...
	if (x != null && y != null)
		if (core.getFlag("碎裂动画", 0) == 1);
		else core.drawAnimate(animate, x, y);
	else
	...
	//大回溯
	if (core.enemys.hasSpecial(special, 43)) { core.status.hero.statistics.battleDamage += core.status.hero.hp *= 1.5 }
	//回魔
	if (core.hasItem("I365")) {
		core.status.hero.statistics.battleDamage += core.status.hero.mana += 1;
		core.status.hero.statistics.battleDamage += core.status.hero.hp += 10
	}
	//负伤宝石
	if (core.hasItem("I532")) { core.status.hero.statistics.battleDamage += core.status.hero.hp += 0.4 * hero.atk + 0.4 * hero.def }
	...
	*/
	...
	return [
		...
		[29, "减伤", function (enemy) { return "无视对手" + (enemy.W || 0) + "%的伤害"; }, "red"],
		[30, "反弹", function (enemy) { return "将对手" + (enemy.fantan || 0) + "%的伤害反弹回去"; }, "blue"],
		[31, "化元", "无视金杖、寒杖、盗贼手套效果", "green"],
		...
		[35, "抓伤", "勇者每回合额外受到怪物攻击力和防御力之和1%的伤害", "red"],
		...
		[43, "大回溯", "战后回复50%的生命值", "#FFC0CB"],
		[44, "夺攻", "夺取勇者10%攻击力加到怪物身上", "red"],
		[45, "夺防", "夺取勇者20%防御力加到怪物身上", "blue"],
		[46, "汲魔", "夺取勇者20%的护盾，怪物生命值提升勇者200%护盾值", "green"],
	...
	//复制
	if (core.hasSpecial(mon_special, 34) && mon_atk < hero_atk * 0.8) {
		mon_atk = hero_atk * 0.8;
		mon_def = hero_def * 0.8;
	}
	// 坚固
	if (core.hasSpecial(mon_special, 3) && mon_def < hero_atk - 1) {
		mon_def = hero_atk * 0.75 + hero_def * 0.25 - 1;
	}
	...
	//史莱姆天赋：缓冲
	if (core.getFlag("技能强化", 0) == 1 && core.hasItem("I1311")) {
		hero_mdef += (Math.min(hero_atk + hero_def, hero_hp * 0.02))
	} else if (core.hasItem("I1122")) {
		hero_mdef += (Math.min(hero_atk + hero_def, hero_hp * 0.01))
	}
	// 技能的处理
	...
	if (core.hasSpecial(mon_special, 44)) {
		hero_atk *= 0.9;
		mon_atk += hero_atk * 0.1
	}
	//夺防

	if (core.hasSpecial(mon_special, 45)) {
		hero_def *= 0.8;
		mon_def += hero_def * 0.2
	}
	//汲魔
	...
	var counterDamage = 0;
	if (core.hasSpecial(mon_special, 8) && !core.hasItem("I532"))
		counterDamage += Math.floor((enemy.counterAttack || core.values.counterAttack) * hero_atk);

	// 先攻
	if (core.hasSpecial(mon_special, 1) && !core.hasItem("I486")) init_damage += per_damage;

	// 破甲
	if (core.hasSpecial(mon_special, 7) && !core.hasItem("I532"))
	...
	if (core.hasItem("I498")) { init_damage -= Math.floor((enemy.purify || core.values.purify) * hero_mdef * 0.5) }
	// 勇士每回合对怪物造成的伤害
	var hero_per_damage = Math.max(hero_atk - mon_def, 0);
	if (core.hasItem("cross")) { hero_per_damage *= 1.15 }
	//减伤
	if (core.hasSpecial(mon_special, 29)) { hero_per_damage *= (1 - (enemy.W / 100)) }
	//反弹
	if (core.hasSpecial(mon_special, 30)) {
		per_damage += (enemy.fantan / 100) * hero_per_damage;
	...
	var mon_turn = Math.sqrt(turn);
	if (core.hasItem("I361")) { damage -= mon_hp * mon_turn * 0.2 }
	if (core.getFlag('hard') == 1) { damage *= 0.7 }
	if (core.hasItem("superPotion")) { damage -= hero_def * 0.3 }
	...
	statusList.forEach(function (item) {
		...
		if (core.hasItem('I489') && item == 'atk') core.setStatusBarInnerHTML(item, core.getRealStatus(item) + '+' + Math.floor(core.getRealStatus('mdef') * 0.05));
		if (core.hasItem('I489') && item == 'def') core.setStatusBarInnerHTML(item, core.getRealStatus(item) + '+' + Math.floor(core.getRealStatus('mdef') * 0.05));
	});
